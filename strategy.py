...
class MultiCurrencyRSIBinaryOptionsStrategy:
    ...
    def check_valid_pairs(self):
        ...
        for pair in self.forex_pairs:
            ...
            for option_type in ["turbo", "binary"]:
                ...
                # Si NO hay prioridad, probar primero las OTC (más estables)
                if PRIORITY_SUFFIX is None:
                    # Orden recomendado: OTC primero (más estable), luego estándar, luego -op
                    variants_to_check.append(f"{pair_upper}-OTC")
                    variants_to_check.append(pair_upper)
                    variants_to_check.append(f"{pair_upper}-op")
                else:
            ...
            if available_options:
                # Si no hay prioridad, preferir OTC para estabilidad
                if PRIORITY_SUFFIX is None:
                    # Ordenar: OTC primero, luego binary sobre turbo
                    available_options.sort(key=lambda x: (
                        not x['is_otc'],  # OTC primero (False=0, True=1)
                        x['option_type'] != 'binary'  # binary preferido sobre turbo
                    ))
    ...
    def process_expired_order(self, pair, order):
        """Procesar una orden expirada"""
        try:
            # Obtener resultado de la orden
            order_result = self.api_call_with_timeout(
                self.iqoption.get_async_order,
                order["id"]
            )
            
            # Si no hay resultado, intentar con check_win_v3
            if order_result is None:
                self.logger.info(f"🔄 Intentando check_win_v3 para orden {order['id']}")
                win_amount = self.api_call_with_timeout(
                    self.iqoption.check_win_v3,
                    order["id"]
                )
                ...
                if win_amount is not None:
                    bet_size = order["size"]
                    if win_amount > 0:
                        # Es una ganancia, el win_amount es el retorno total
                ...
                self.logger.warning(f"⚠️ No se pudo obtener resultado para orden {order['id']}")
                time_since_expiry = (datetime.now() - order["expiry_time"]).total_seconds()
                if time_since_expiry < 30:
                    self.logger.info(f"⏳ Orden muy reciente, se volverá a verificar en el próximo ciclo")
                    return  # No procesar aún
                # Si ya pasó mucho tiempo, asumir pérdida
                self.logger.error(f"❌ No se pudo verificar orden {order['id']} después de {time_since_expiry:.0f}s")
                self.process_loss(pair, order)
                return
            
            # Log del resultado para debug
            self.logger.info(f"📋 Resultado de orden {order['id']}: {order_result}")
            
            # Determinar el resultado basado en los campos disponibles
            bet_size = order["size"]
            is_win = False
            is_tie = False
            win_amount = 0
            
            # Método 1: Campo 'win' directo (más confiable)
            if "win" in order_result:
                ...
                if win_status == "win":
                    ...
                    if "win_amount" in order_result:
                        win_amount = order_result["win_amount"]
                    elif "win_enrolled_amount" in order_result:
                        win_amount = order_result["win_enrolled_amount"]
                    else:
                        # Asumir payout estándar del 80%
                        win_amount = bet_size * 1.80
                elif win_status == "equal":
                    is_tie = True
                    win_amount = bet_size
                else:  # "loose" o cualquier otro valor
                    is_win = False
                    win_amount = 0
            
            # Método 2: Verificar win_amount vs loose_amount
            elif "win_amount" in order_result and "loose_amount" in order_result:
                win_amt = order_result.get("win_amount", 0)
                loose_amt = order_result.get("loose_amount", 0)
                
                if win_amt > 0 and loose_amt == 0:
                    is_win = True
                    win_amount = win_amt
                elif win_amt == 0 and loose_amt > 0:
                    is_win = False
                    win_amount = 0
                elif win_amt == bet_size and loose_amt == 0:
                    is_tie = True
                    win_amount = bet_size
            
            # Método 3: Solo win_amount
            elif "win_amount" in order_result:
                win_amount = order_result.get("win_amount", 0)
            ...
            # Método 4: profit_amount
            elif "profit_amount" in order_result:
                profit = order_result.get("profit_amount", 0)
                if profit > 0:
                    is_win = True
